...
$con = $conn->connect();
    ...
    if( !empty( $param['search']['value'] ) ){
        $search='';
        $search=" AND ( lgsa_control_no like '%".$param['search']['value']."%' ";
    ...
    if($param['order'][0]['column']!=''){
        $cols  = array('lgsa_control_no','lgsa_payee_full_name','lgsa_voucher_prepared_by_name','lgsa_voucher_approved_by_name','lgsa_voucher_status');
        if($param['length']==-1){
    ...
    if(!empty($param['accountuserid'])){
        ...
        if(mysqli_num_rows($rs)>0){
            ...
            while($row=mysqli_fetch_array($rs)){
                ...
                $data["data"][] = array(
                    'id'=>$i,
                    'user_id'=>$row['lgsa_voucher_id'],
                    'control_no'=>$row['lgsa_control_no'],
                    'voucher_date'=>$row['lgsa_voucher_date'],
                    'payee_fullname'=>$row['lgsa_payee_full_name'],
                    'total'=>$row['lgsa_voucher_total'],
                    'prepared_by'=>$row['lgsa_voucher_prepared_by_name'],
                    'approved_by'=>$row['lgsa_voucher_approved_by_name'],
                    'approved_date'=>$row['lgsa_voucher_approved_date'],
